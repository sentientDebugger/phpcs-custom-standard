...
/**
 * Based from Generic.WhiteSpace.DisallowTabIndent
 * @author Carlos Enrique Sifuentes <me@carlos-sifuentes.com>
 * @see https://github.com/squizlabs/PHP_CodeSniffer/blob/master/src/Standards/Generic/Sniffs/WhiteSpace/DisallowTabIndentSniff.php
 */
class DisallowSpaceIndentSniff implements Sniff
{
	...
	public function process(File $phpcsFile, $stackPtr)
	{
		...
		for ($i = 0; $i < $phpcsFile->numTokens; $i++) {
			...
			$foundSpaces = substr_count($content, str_repeat(' ', $this->tabWidth));

			$error     = 'Tabs must be used to indent lines; spaces are not allowed';
			$errorCode = 'SpacesUsed';
			$allowSpacesForAlignment = true;
			if ($tokens[$i]['column'] === 1) {
			...
			} elseif ($allowSpacesForAlignment) {
				continue;
			} else {
				...
				// line indent tokens.
				if ($foundSpaces > 0) {
					$error     = 'Tabs must be used for alignment; spaces are not allowed';
					$errorCode = 'NonIndentSpacesUsed';
				}
			}//end if
